...
CREATE TABLE IF NOT EXISTS `notification` (
    ...
    `sender_id`                 BIGINT NOT NULL,                                                                        -- 触发者ID
    `type`                      ENUM('LIKE', 'COMMENT', 'FOLLOW', 'SYSTEM') NOT NULL,                                   -- 通知类型
    `article_id`                BIGINT DEFAULT NULL,                                                                    -- 关联文章ID（如点赞、评论，NULL表示没有关联文章）
...
CREATE TABLE IF NOT EXISTS `bookmark` (
    ...
    INDEX index_collector_id    (collector_id),
    INDEX index_article_id      (article_id)
